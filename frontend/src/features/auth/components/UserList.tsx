...
export const UserList = () => {
  ...
  const baseColumns: TableColumnsType<UserResponse> = [
    ...
    },
    {
      ...
      dataIndex: 'phone',
      key: 'phone',
      render: (phone: string) => phone || <span className="text-gray-400">Chưa cập nhật</span>,
    },
    {
      title: 'Vai trò',
      dataIndex: 'roles',
    ...
    columns.push({
      ...
      render: (_: any, record: UserResponse) => (
        <Space size="middle">
          ...
          {record.isActive && record.email !== 'admin@laundry.local' && canDelete && (
            <Popconfirm
    ...
    <div className="space-y-4">
      ...
      >
        <Form form={createForm} layout="vertical" className="mt-4">
          ...
          >
            <Input placeholder="user@laundry.local" />
          </Form.Item>
        ...
        <Form form={editForm} layout="vertical" className="mt-4">
          ...
          >
            <Select
              ...
              options={activeRoles?.map(r => ({ label: r.name, value: r.id }))}
              style={{ width: '100%' }}
              disabled={selectedUser?.email === 'admin@laundry.local'}
            />
          </Form.Item>
        </Form>
      </Modal>
